...
Description: Ums Pay for WooCommerce is a payment gateway plugin for WooCommerce. that allows you to accept payments via Mpesa Stk Push directly on Mpesa Till, Paybill and Bank Account.
Author: Alvin Kiveu (UMESKIA SOFTWARES)
Author URI: https://github.com/alvin-kiveu/
Version: 1.0.0
Text Domain: https://portal.umeskiasoftwares.com/
...
function add_umspay_gateway($methods)
{
  $methods[] = 'WC_UmsPay_Gateway';
  return $methods;
}
...
add_action('plugins_loaded', 'umspay_woocommerce_init', 0);

function umspay_woocommerce_init()
{
  if (class_exists('WC_Payment_Gateway')) {
    ...
    class WC_UmsPay_Gateway  extends WC_Payment_Gateway
    {
      public function __construct()
      {
        session_start();
        $this->id = 'umspay';
        $this->icon = apply_filters('woocommerce_umspay_icon', plugin_dir_url(__FILE__) . 'umspaylogo.png');
